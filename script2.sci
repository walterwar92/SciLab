...
function [L, U, P] = LU_decomposition(A)
    ...
    P = eye(n);
    for k = 1:n-1
        // Проверим, чтобы индекс k оставался в пределах диапазона
        if k > n then
            error("Индекс k выходит за пределы диапазона.");
        end

        // Найдем максимальный элемент в столбце k с частичным выбором ведущего элемента
        [max_value, i_max] = max(abs(U(k:n, k))); // Индекс строки с максимальным элементом
        i_max = i_max + k - 1;  // Индекс для строки из подматрицы, скорректированный на k

        // Проверка, что индекс i_max находится в допустимом диапазоне
        if i_max > n then
            error("Индекс i_max выходит за пределы диапазона.");
        end
        ...
        // Нормальное выполнение LU-разложения
        for i = k+1:n
            L(i, k) = U(i, k) / U(k, k);
